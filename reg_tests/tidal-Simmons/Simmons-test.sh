...
CVMix=$PWD/../..
make -f $CVMix/src/Makefile ${USE_NETCDF} CVMIX_DRIVER=cvmix_Simmons-tidal_driver.F90
# Note: if make error, include CVMIX_ROOT as below
#make -f $CVMix/src/Makefile ${USE_NETCDF} CVMIX_DRIVER=cvmix_Simmons-tidal_driver.F90 CVMIX_ROOT=$CVMix
if [ $? != 0 ]; then
  echo "Build error!"
  exit 1
fi
